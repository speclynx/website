...
- **Review** — on the homepage, one per testimonial card (real, attributed quotes only; never invented or placeholder quotes): `itemReviewed` references the Organization `@id`, `author` is a Person with `jobTitle`, `image`, and `worksFor` Organization. Keep `reviewBody` and author details in sync with the visible cards; no `reviewRating` (testimonials carry no score). Testimonial photos live in `assets/images/testimonials/` as WebP
